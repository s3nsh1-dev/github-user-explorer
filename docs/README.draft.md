<!--
  DRAFT — not the live README.

  `README.md` at the repo root is untouched on purpose. Read this, change what
  you disagree with, then move it into place:

      git mv docs/README.draft.md README.md

  ⚠️ Every path below is written for the ROOT, which is where this is going —
  so the images will not preview while the file is still in docs/. They resolve
  correctly the moment you move it.

  Every claim below was checked against the code or measured in a browser. If
  you edit it, keep that property — an unverifiable number in a README is worse
  than no number.
-->
